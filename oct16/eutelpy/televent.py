...
class TelEvent(object):
  ...
  ## Analyze event.
  #
  # Look at (unbiased) residuals
  #
  def analyzeRes2(self):
    for it, t in enumerate(self.__tracks):
      hits = t.getHits()
      traj = self.__traj[it]
      for p, h in hits.iteritems():
        label = h.getLabel()
        # refit track without current hit (point)
        Chi2, Ndf = traj.fit(skipMeasLabel=label)[:2]
        numData, aResiduals, aMeasErr, aResErr, aDownWeight = traj.getMeasResults(label)
        for j in range(numData):
          print " measRes2 " , p, label, j, aResiduals[j], aMeasErr[j], aResErr[j], aDownWeight[j], Chi2, Ndf
        numData, aResiduals, aMeasErr, aResErr, aDownWeight = traj.getScatResults(label)
        for j in range(numData):
          print " scatRes2 " , p, label, j, aResiduals[j], aMeasErr[j], aResErr[j], aDownWeight[j], Chi2, Ndf
