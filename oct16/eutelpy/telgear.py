...
class GearSetup(object):
  ...
  ## get order (in Z) of mimosa planes.
  def getMimosaOrder(self):
    order = [] 
    for planeID in sorted(self.__det.keys(), key=lambda p: self.__det[p].getZintersect()): 
      if planeID < 6:
        order.append(planeID)
    print " Mimosa plane order ", order 
    if len(order) <> 6:
      print " !!! Number of mimosa planes is not six !!!"   
    return order    
