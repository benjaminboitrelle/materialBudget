...
class TelTriplets(object):    
  ...
  #
  def __init__(self, header, planeIDs, order, cuts, hists):
    ## (event) header
    self.__hdr = header 
    ## hits (in layers)
    self.__hits = {}
    for p in planeIDs:
      self.__hits[p] = []
    ## order of mimosa planes
    self.__mimosaOrder = order  
    ## triplets
    self.__triplets = [[], []]   
    ## segments
    self.__seg = []
  ...
  def findTracks(self, hits, qbyp=0., bfac=(0., 0.), zMag=0.):   
    # cuts
    doubletCuts, tripletCuts, slopeCut, positionCut = self.__cuts[:4]
    # curvature
    ...
    nTrp = 0
    for lcenter in [1, 4]:
      # doublet/triplet cuts (per arm)  
      if len(doubletCuts) > 3:
        doubletCut = doubletCuts[lcenter / 2:lcenter / 2 + 2]
        cDoublet = str(lcenter / 4)
      else:
        doubletCut = doubletCuts[:2]
        cDoublet = ''
      #  
      if len(tripletCuts) > 3:
        tripletCut = tripletCuts[lcenter / 2:lcenter / 2 + 2]
        cTriplet = str(lcenter / 4)
      else:
        tripletCut = tripletCuts[:2]
        cTriplet = ''  
      # check for triplets   
      for hit1 in self.__hits[self.__mimosaOrder[lcenter - 1]]:
        for hit3 in self.__hits[self.__mimosaOrder[lcenter + 1]]:
          ...
          if self.__hists is not None:
            self.__hists.addEntry("doubletDx" + cDoublet, dx)
            self.__hists.addEntry("doubletDy" + cDoublet, dy)
          # take doublet
          if tripletCut is None:
            self.__triplets[lcenter / 3].append(TelTriplet([hit1, hit3], aDoublet, None))
            continue
          # validate triplet  
          for hit2 in self.__hits[self.__mimosaOrder[lcenter]]:
            ...
            if trp is not None:
              ...
              if self.__hists is not None:
                self.__hists.addEntry("tripletDx" + cTriplet, trp[0])
                self.__hists.addEntry("tripletDy" + cTriplet, trp[1])
  ...
  def matchDUT(self):
    ...
    for l, hits in self.__hits.iteritems():
      ...
      dutType = l / 10 - 1
      if len(dutCut) > 2:
        distCut = dutCut[2 * dutType:2 * dutType + 2]
        cType = str(dutType) 
      else:
        distCut = dutCut
        cType = ''  
      #print " DUT type ", l, dutType, distCut  
      # look for unique matches
      matches = []
      ...
      for i, h in enumerate(hits):  
        ...
        for j, seg in enumerate(self.__seg):
          ...
          if dx < distCut[0] and dy < distCut[1]:
            ...
            seg.addMatch()
            if self.__hists is not None:
              self.__hists.addEntry("DUT-dx" + cType, x - px)
              self.__hists.addEntry("DUT-dy" + cType, y - py)
